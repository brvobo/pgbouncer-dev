...
static void start_auth_request(PgSocket *client, const char *username)
{
	...
	db = find_database(cf_auth_conn);
	if (!db) {
		disconnect_client(client, true, "auth_conn requires a matching database %s", cf_auth_conn);
		return;
	}
	if (!db->forced_user) {
		disconnect_client(client, true, "auth_conn %s requires a dedicated user", cf_auth_conn);
		return;
	}
	client->pool = get_pool(db, db->forced_user);
...
static bool finish_set_pool(PgSocket *client, bool takeover)
{
	...
	if (client->db->forced_user) {
		if (client->db->force_user) {
			if (strcmp(client->auth_user->name, client->db->forced_user->name) != 0) {
				disconnect_client(client, true, "User %s not allowed to connect", client->auth_user->name);
				if (cf_log_connections)
					slog_info(client, "login failed: db=%s user=%s", client->db->name, client->auth_user->name);
				return false;
			}
...
bool handle_auth_response(PgSocket *client, PktHdr *pkt) {
	...
	case 'D':	/* DataRow */
		...
		client->auth_user = add_user(user.name, user.passwd);
		if (!client->auth_user) {
